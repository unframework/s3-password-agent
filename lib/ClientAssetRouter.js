...
function whenClientCodeCompiled(auth0Settings, sourceCode, basedir) {
    ...
    b.exclude(AUTH0_EXPOSE_NAME).require(settingsRC, { expose: AUTH0_EXPOSE_NAME, basedir: __dirname });

    if (auth0Settings) {
        b.require('auth0-lock');
    }
...
function ClientAssetRouter(auth0Settings, clientMainFile, basedir) {
    ...
    var whenReady = whenClientCodeLoaded(clientMainFile).then(function (fileData) {
        return whenClientCodeCompiled(auth0Settings, fileData, basedir).then(function (code) {
            console.info('compiled client code:', path.basename(clientMainFile));
            return code;
        });
    });
